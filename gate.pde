    ...
    void checkClick() {
      ...
      if (currentTool == 0 && mouseY > -5 * Scale + posY && mouseY < 10 * Scale + posY - Scale*Scale) {
        if (mouseX > -10 * Scale + posX && mouseX <= Scale * 15 + posX - Scale*Scale*2) {
          ...
          if (selected == false && millis() - prevMillis > 100) {selected = true; prevMillis = millis(); println(1234567890);}
          if (selected == true && millis() - prevMillis > 100) {
            selected = false; 
            prevMillis = millis();
            
            for (int Gate = 0; Gate < selectedID.size(); Gate++) {
              if (gates.get(selectedID.get(Gate)).ID == ID) {
                println(123);
                selectedID.remove(Gate);
              }
            }
          }
        }
        ...
        for (int Gate = 0; Gate < selectedID.size() && selected == true; Gate++) {
          if (Gate < selectedID.size() && gates.get(selectedID.get(Gate)).ID == ID) {toAdd = false;}
        }
